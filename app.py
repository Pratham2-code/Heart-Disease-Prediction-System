    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if shap_explainer:
                ...
                for i, name in enumerate(FEATURE_NAMES):
                    if isinstance(val, np.ndarray):
                        if val.ndim > 1:
                            # Handles multi-class or binary explanation arrays (e.g. from RandomForest)
                            xai_data['shap'][name] = float(val[i][1])
                        else:
                            # Handles single probability contribution array (e.g. from LogisticRegression)
                            xai_data['shap'][name] = float(val[i])
                    else:
                        xai_data['shap'][name] = float(val) if i == 0 else 0.0
            ...
            if lime_explainer:
                ...
                # Extract weights using feature indices for 100% accuracy
